...
def create_plot_date_iops(trace_file):
    global _plot_col
    global _time_col
...
def main(argv):
    ...
    for trace_file in argv[1:] :
        try:
            ...
            pl.plot(x,y, label=trace_file)
        except:
            print("Error, Plotting %s"%(trace_file) )
            pass
    pl.savefig(trace_file + ".latency.png")
    pl.clf()
    trace_file = ""
    ...
    for trace_file in argv[1:] :
        try:
            x , y = create_plot_date_iops(trace_file)
            pl.plot(x,y, label=trace_file)
        except:
            print("Error, Plotting %s"%(trace_file) )
            pass
    pl.savefig(trace_file + ".iops.png")
    pl.clf()
