...
for i in $@
	do
		...
		gnuplot -e " 	set ylabel '$YLABLE';\
				...
				set output '${i}.Load.png' ;\
				set grid x y2;\
				set key top right outside;\
				set lmargin 12;\
				set rmargin 24;\
				set datafile separator ',' ;\
				plot '${i}.arr' using 2 axes x1y1 title 'Inter-Arrival Time' with points lt 4 pt 6 ps 0.5 , '${i}.arr' using 3 axes x1y2 title 'Controller Load' with l lt 2 , '${i}.HostLoad' using 1 axes x2y2 title 'Host Load' with l lt 3  ;"
				rm ${i}.arr
	done 
#				set logscale y ;\
#				set xrange [1:*] ;\
