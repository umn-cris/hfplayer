...
void cp_stat_files(){
	...
	sprintf(command,"cp %s ./%s", statname , "stat");
	system(command);

	sprintf(statname,"/proc/%d/schedstat",pid);
	sprintf(command,"cp %s ./%s", statname , "schedstat");
	system(command);
	
	sprintf(statname,"/proc/%d/sched",pid);
	sprintf(command,"cp %s ./%s", statname , "sched");
	system(command);
...
int main(int argc, char *argv[])
{
	...
	assert(pgsize > 0);
        ...
        if (sched_setscheduler(0, SCHED_RR, & param) != 0) {
                 fatal("sched_setscheduler", ERR_SYSCALL, "Can't set task scheduler info\n");
        }
