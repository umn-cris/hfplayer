...
SKIP_IO=0
# functions 
RESDIR="/home/alireza/bundling_traces"
...
SDs="sdb"
mkdir $RESDIR
HFbundles="0 10000 20000 30000 40000 50000 60000"
#HFbundles="30000"
BTbs="1 5 10 15 20"
NTHREADS="8"
...
for SD in $SDs ; do 
    ...
    for TRACE in "sdb.fio.sr.2048" ; do 
        for nt in $NTHREADS ; do 
            for hfb in $HFbundles ; do 
                echo -e "\n=====---- Start hfplayer $TRACE on $SD with $hfb bundle size with $nt threads  ----======"
                run_hfplayer $SD $TRACE $hfb $nt
            done
        done 
        #for btb in $BTbs ; do
