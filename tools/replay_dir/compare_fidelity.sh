...
function run_hfplayer () {
    ...
    mv *.hfplayer $RESDIR/$TRACE.$NT.$BT.hfplayer
}
#--------------------------------------#
function run_blkreplay () {
    SD=$1
    TRACE=$2
    BLKTR=$3
    rm origs/*.load >> /dev/null 2>&1
    ./netapp_to_load.py origs/$TRACE
    cmd="cat origs/${TRACE}.load | blkreplay.exe --replay-start=0 --replay-end=0 --with-conflicts --threads=$BLKTR --no-overhead /dev/$SD "
    run_and_capture $cmd 
    rename inout blkreplay
    mv *.blkreplay $RESDIR/$TRACE.$BLKTR.blkreplay
    rm origs/*.load >> /dev/null 2>&1
}
# Initialize:
#--------------------------------------------------------#
...
for SD in $SDs ; do 
    ...
    #for TRACE in `cd origs; ls sdi.fio.*` ; do 
    for TRACE in "sdi.fio.sw.128 sdi.fio.randrw.512 sdi.fio.randrw.128 sdi.fio.randrw.32" ; do 
        for btr in $BLKTHREADS ; do 
            echo -e "\n=====---- Start blkreplay $TRACE on $SD with $btr threads   ----======"
            run_blkreplay $SD $TRACE $btr
        done 
