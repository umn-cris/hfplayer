...
function run_hfplayer () {
    ...
    NT=$4
    MODE=$5
    cmd="./hfplayer -mode $MODE -nt $NT -cfg ReplayCfg.csv.$SD -q 2048 -b $BT origs/$TRACE"
    #./monitor_cpu.sh hfplayer >> /dev/null  &
    run_and_capture $cmd 
    rename inout hfplayer 
    mv *.hfplayer $RESDIR/$TRACE.$NT.$BT.hfplayer.$MODE.$SD
}
# Initialize:
#--------------------------------------------------------#
...
#echo "--------------------------------------------------------"
#mode="dep"
#for TRACE in `cd origs; ls sdd.copyfiles*.annot` ; do 
#        echo -e "\n=====---- Start hfplayer $TRACE on sdi with $mode mode  ----======"
#        run_hfplayer sdd $TRACE 0 1 $mode
#done
##--------------------------------------------------------#
