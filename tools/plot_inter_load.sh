...
for i in $@
	do
		awk 'BEGIN { FS = "," ; OFS = "," }; {print $1,$3-i,$6; i=$3}' $i | sed '1,2d' >$i.temp
		...
		gnuplot -e " 	set ylabel '$YLABLE';\
				...
				plot '${i}.temp' using 2 axes x1y1 title 'Inter-Arrival Time' with points lt 4 pt 6 ps 0.5 , '${i}.temp' using 3 axes x1y2 title 'Controller Load' with l lt 2  ;"
done 
#				set logscale y ;\
#				set xrange [1:*] ;\
#				set yrange [1:$YMAX] ;\
